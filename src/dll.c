...
void
dll_destroy (dll_t **dll, void (*destroy) (void *))
{
  if (dll == NULL)
    {
      goto ret;
...
int
dll_insert_front (dll_t *dll, void *data)
{
  int ret_val = -1;
...
int
dll_insert_rear (dll_t *dll, void *data)
{
  int ret_val = -1;
...
int
dll_insert_at_pos (dll_t *dll, size_t index, void *data)
{
  dll_node_t *new = NULL;
...
int
dll_delete_at_pos (dll_t *dll, size_t index)
{
  dll_node_t *temp = dll->head;
  int ret_val = -1;
  size_t count = 0;
...
dll_node_t *
dll_search (dll_t *dll, void *data)
{
  dll_node_t *temp = dll->head;
  while (temp)
    {
...
int
dll_insertion_sort (dll_t *dll, dll_node_t **head_ref, void *data, int (*compare) (void *a, void *b))
{
  int ret_val = -1;
  int compare_result;
